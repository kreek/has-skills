...
const VERSIONS = {
  ...
  "agent-booster-pack-specify": "1.0.0",
...
describe("Pi package npm publish script", () => {
  ...
  it("publishes missing Pi packages in dependency order", () => {
    ...
    expect(publishLines).toEqual([
      ...
      "npm publish ./agent-booster-pack-specify --access public",
  ...
  it("dry-run reports missing packages without publishing", () => {
    ...
    expect(result.stdout).toContain("would publish agent-booster-pack-specify@1.0.0");
  ...
  it("waits until each published version is visible", () => {
    ...
    const specifyPublish = lines.indexOf("npm publish ./agent-booster-pack-specify --access public");
    const specifyConfirm = lines.indexOf(
      "npm view agent-booster-pack-specify@1.0.0 version",
      specifyPublish + 1,
    );
    ...
    expect(skillsConfirm).toBeLessThan(specifyPublish);
    expect(specifyPublish).toBeLessThan(specifyConfirm);
    expect(specifyConfirm).toBeLessThan(metaPublish);
  });
