...
1. Initialize git before feature code in fresh scaffolds, create a
...
2. Pick one package manager and commit its lockfile. Use the modern
...
3. Standardize task names: `test`, `lint`, `format`, `typecheck`,
   `coverage` where applicable. CI runs the same commands developers
   run locally.
4. Satisfy named artifacts literally before applying ecosystem
...
5. A `typecheck` script must run a type checker or the repo's
...
6. For fresh web apps, use a mature framework with routing, request
...
7. If the language, runtime, app shape, or ecosystem state isn't
   covered by `references/`, search current official sources before
   choosing; explain the chosen default in one sentence.
8. Add one smoke test that proves the runner, import path, and build
   system work together. The first real feature should not need
   tooling decisions.
9. README says what it is, how to run it, and how to test it.
