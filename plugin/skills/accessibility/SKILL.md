...
2. Every interactive element needs name, role, value, state, and
   keyboard behavior.
3. Focus must be visible (≥2px outline at 3:1 contrast vs both
   adjacent surfaces, with a transparent fallback for Forced
   Colors), ordered, restored after modal/overlay close, and never
   trapped by hand inside a `<dialog>`.
...
| "Custom focus trap inside our modal" | Use native `<dialog>` + `.showModal()`. Do not trap focus by hand; the user agent prevents tab from leaving into the document and intentionally allows escape to browser chrome. | A pre-`<dialog>` legacy code path explicitly documented as such, with a migration plan. |
