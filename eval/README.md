...
This eval suite gates Consult changes against regressions and, when a new
comparison run is wanted, benchmarks whether Codex behaves better when
Consult is installed as a Codex plugin.

**Default workflow: regression.** Consult-vs-baseline behaviour is established;
routine change-validation runs `regression` against `codexWithConsultSkills`
...
- `codexWithConsultSkills`: the same Codex model with the local repo registered
  as a codex plugin marketplace and the `consult@consult` plugin enabled — the same
...
- `allSkills`: a larger suite intended to exercise every Consult skill listed in
...
# Regression — the default workflow. Runs codexWithConsultSkills only.
...
pnpm run regression:all   # full Consult-only sweep

pnpm run trial -- proof-first-bugfix --profile codexWithConsultSkills

# Bench — cross-profile comparison. Reserved for re-baselining.
pnpm run bench:smoke      # compare Codex baseline vs Codex + Consult
pnpm run bench:core
...
Use **Regression** for routine Consult change-validation: one profile (Consult),
one suite, score against history. Use **Bench** when a fresh
Consult-vs-baseline comparison is needed (every release, or when claiming
a directional win against unaided Codex). Use **Trial** to debug one
task under one profile.

Results are written under `~/.cache/consult/eval/runs/` by default
(override with `CONSULT_EVAL_RUNS_DIR`). Trial workdirs live outside the repo to
keep codex's ancestor walk from auto-discovering Consult skills into the baseline
profile.
