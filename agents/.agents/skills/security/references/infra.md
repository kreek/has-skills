...
- **`trivy config`** / **`checkov`** for Terraform. Treat **`tfsec`** as
  legacy; it is now part of Trivy.
...
- **OIDC for cloud auth from CI** (GitHub Actions → AWS, GCP, Azure;
  GitLab equivalent). Eliminates long-lived static keys.
...
- **Sign artifacts.** Sigstore / cosign for container images and binaries;
  ...
  admission.
- **SBOM** per release (`syft`, `cdxgen`); store with the artifact;
  required for incident response when a CVE drops.
