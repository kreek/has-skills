...
- Query optimization without cached data; use `database` or `performance`.
...
- Use `resilience` when cached data crosses service boundaries or participates
  in retry/fallback behavior.
- Use `security` when cache keys/values can include tenant data, secrets,
